...
Joystick::Joystick(uint8_t left_pin, uint8_t right_pin, uint8_t up_pin,
                   uint8_t down_pin, uint8_t press_pin)
    : left_pin_(left_pin), right_pin_(right_pin), up_pin_(up_pin),
      down_pin_(down_pin), press_pin_(press_pin) {
...
JoystickPosition Joystick::getPosition() {
  ...
  if (digitalRead(this->left_pin_) == LOW) {
    if (this->last_position_ != JoystickPosition::LEFT ||
    ...
    if (this->last_position_ != JoystickPosition::RIGHT ||
        (millis() - this->last_print_time_) > this->hold_time_) {
    ...
    if (this->last_position_ != JoystickPosition::UP ||
        (millis() - this->last_print_time_) > this->hold_time_) {
    ...
    if (this->last_position_ != JoystickPosition::DOWN ||
        (millis() - this->last_print_time_) > this->hold_time_) {
...
ButtonState Joystick::getState() {
  ...
  if (this->recent_double_click_) {
    if (millis() - this->last_double_click_time_ >
        this->double_click_timeout_time_) {
      this->recent_double_click_ =
          false; // Go back to an mode where we can publish stuff
    }
  }
  if (this->recent_initial_click_ && !this->recent_double_click_) {
    if (millis() - this->last_push_release_time_ > this->double_click_time_ &&
        digitalRead(this->press_pin_) ==
            HIGH) { // Enough time has elapsed since last
    ...
    } else if (digitalRead(this->press_pin_) == LOW) { // Joystick is pressed
      this->last_push_ = true;
      state =
          ButtonState::DOUBLE; // A second click, so publish the double click
      this->recent_initial_click_ = false;
      this->recent_double_click_ = true;
      this->last_double_click_time_ = millis();
    } else { // Joystick not pressed, not just released and recent enough
             // initial click
      this->last_push_ = false;
      state = ButtonState::NEUTRAL;
    }
  } else { // No recent initial click or a recent double click
    if (digitalRead(this->press_pin_) == LOW) { // Pressed
      this->last_push_ = true;
    } else {                  // Not pressed
      if (this->last_push_) { // Just released
